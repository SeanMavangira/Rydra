...
struct IncomeChartView: View {
    ...
    var groupedByDay: [(String, Int)] {
        ...
        let grouped = Dictionary(grouping: viewerModel.incomes) { expense in
    ...
    var groupedByWeek: [(String, Int)] {
        ...
        return grouped
            .map { (key, value) in
                (key, value.reduce(0) { $0 + $1.amountPaid })
            }
            .sorted(by: { $0.0 < $1.0 })
    }
    ...
    var groupedByMonth: [(String, Int)] {
        ...
        let grouped = Dictionary(grouping: viewerModel.incomes) { income in
